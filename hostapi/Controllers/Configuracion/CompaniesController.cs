...
namespace hostapi.Controllers.Configuracion
{
    ...
    public class CompaniesController : ApiController
    {
        ...
        [Route("companies")]
        public async Task<HttpResponseMessage> Companies(CompaniesDataModel model)
        {
            Thread.CurrentThread.CurrentCulture = culture;

            List<CompaniesQueryModel> resultSeachModel = null;
            CompaniesByIdModel resultSeachByIdModel = null;
            Companies proxy = new Companies();
            ...
            try
            {
                if (!await Authentication.isAdmin(User, Request))
                {
                    ...
                    if (!await auth.AccesRights(User.Identity.GetUserId(), "companies", model.type))
                    {
                ...
                using (TransactionScope scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    using (SqlConnection connection1 = new SqlConnection(connetionString))
                    {
                        ...
                        if (model.type == 1)
                        {
                            resultSeachModel = new List<CompaniesQueryModel>();
                            await proxy.SearchQuery(model.search, resultSeachModel, connection1);
                        }
                        else if (model.type == 2)
                        {
                            resultSeachByIdModel = new CompaniesByIdModel();
